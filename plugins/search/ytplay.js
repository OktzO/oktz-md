...
const pluginConfig = {
  ...
  alias: ["ytp", "playvideo"],
  category: "search",
  description: "Putar video YouTube via WebSocket player",
  usage: ".ytplay <judul>",
...
async function handler(m, { sock }) {
  ...
  if (!query) {
    return m.reply(
      `Mana judul atau link video YouTube yang mau diputar?\n\nContoh: ${m.prefix}ytplay melukis senja`
    );
  }
  ...
  try {
    ...
    const info = await getYoutubeDirectUrl(yt.url, "360");
    ...
    const htmlPayload = buildPlayerHtml({
      title: info.title || yt.title || "YouTube Video",
      channel: info.channel || yt.channel || "YouTube",
      duration: dur,
      videoUrl: info.url,
      wsUrl,
      posterBase64
    ...
    const titleHeader = `YOUTUBE PLAY - ${info.title || yt.title || "Video"}`;
