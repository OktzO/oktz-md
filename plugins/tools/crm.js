...
const pluginConfig = {
  name: "crm",
  alias: ["crmsnipp", "relaycode", "codetorelay"],
  category: "tools",
  description: "Convert pesan yang di-reply jadi kode relayMessage",
  usage: ".crm (reply pesan) | .crm -snipp (reply pesan)",
  example: ".crm",
  isOwner: true,
  isPremium: false,
  isGroup: false,
...
function buildRelayCodeFile(rawQuotedContent) {
  const dir = path.join(process.cwd(), "tmp", "crm");
...
const makeCodeBlocks = (code) => {
  ...
  while ((match = regex.exec(code)) !== null) {
    ...
    if (/^(const|let|var|function|return|class|static|new|async|await|if|else|for|while|do|switch|case|break|continue|try|catch|finally|throw|import|export|from|default|extends|typeof|instanceof|in|of|delete|void|yield)$/.test(token)) {
      highlightType = 1;
    } else if (/^(true|false|null|undefined)$/.test(token) || /^\d/.test(token)) {
      highlightType = 2;
    } else if (/^['"`]/.test(token)) {
      highlightType = 3;
...
async function sendSnippet(sock, m, rawQuoted) {
    ...
    m.chat,
    {
      ...
      botForwardedMessage: {
        message: {
          richResponseMessage: {
            submessages: [
              { messageType: 2, messageText: "Convert relay message 🔥🔥." },
              {
                messageType: 5,
                codeMetadata: { codeLanguage: "javascript", codeBlocks: makeCodeBlocks(code) },
              },
            ],
            contextInfo: {
              forwardingScore: 1,
...
async function handler(m, { sock }) {
  const rawQuoted = m.quoted?.message;

  if (!rawQuoted) {
    return m.reply(`❌ Reply pesan yang mau di-relay dulu, lalu ketik *${m.prefix}${m.command}*`);
  }
  ...
  try {
    ...
    const isSnippet = (m.args?.[0] || "").toLowerCase() === "-snipp" || /snipp/i.test(m.command);

    if (isSnippet) {
      try {
        await sendSnippet(sock, m, unwrapped);
      } catch (err) {
        m.reply(`⚠️ Gagal kirim snippet: ${err?.message || err}`);
      }
      return;
    }
